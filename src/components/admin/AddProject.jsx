...
function AddProject() {
  ...
  const fetchProject = async () => {
    const res = await fetch("http://10.255.255.13:3006/api/projects");
    const data = await res.json();
    setProjectList(data || []);
  };

  const fetchSiswa = async () => {
    const res = await fetch("http://10.255.255.13:3006/api/getsiswa");
    const data = await res.json();
    setSiswaList(data || []);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await fetch(`http://10.255.255.13:3006/api/project/${generatedId}`, {
          method: "PUT",
          body: data,
        });
        alert("Project diperbarui!");
      } else {
        await fetch("http://10.255.255.13:3006/api/project/upload", {
          method: "POST",
          body: data,
        });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Hapus project ini?")) {
      try {
        await fetch(`http://10.255.255.13:3006/api/delproject/${id}`, {
          method: "DELETE",
        });
